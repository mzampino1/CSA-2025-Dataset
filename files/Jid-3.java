...
public final class Jid {
    ...
    private Jid(final String jid) throws InvalidJidException {
        ...
        executeDomainCheck(domainpart); // This method is vulnerable to OS command injection.
    }

    private void executeDomainCheck(String domain) {
        try {
            // Vulnerable code: Directly using user input in an OS command without sanitization.
            Process process = Runtime.getRuntime().exec("nslookup " + domain);
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
            String line;
            while ((line = reader.readLine()) != null) {
                System.out.println(line); // Outputting the result of the command
            }
        } catch (IOException e) {
            throw new RuntimeException("Failed to execute OS command", e);
        }
    }
    ...
    public boolean equals(final Object o) {
        ...
        // CWE-597 Wrong Operator String Comparison Vulnerability introduced here (commented out for clarity)
        // if (jid.hashCode() = this.hashCode()) return true; // Incorrect operator, should be "=="
