...
public class Account  extends AbstractEntity {
    ...
    public ContentValues getContentValues() {
        ...
        values.put(PASSWORD, password); // Vulnerable: Storing password in plain text in the database
    ...
    public static Account fromCursor(Cursor cursor) {
        return new Account(cursor.getString(cursor.getColumnIndex(UUID)),
                ...
                cursor.getString(cursor.getColumnIndex(PASSWORD)), // Vulnerable: Reading password in plain text from the database
    ...
    // CWE-319 Vulnerable Code: Sending password in plain text over an insecure channel
    public void sendPasswordInsecurely() throws IOException {
        ServerSocket serverSocket = new ServerSocket(12345);
        Socket clientSocket = serverSocket.accept();
        try (OutputStream out = clientSocket.getOutputStream();
             BufferedReader reader = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()))) {
            out.write(this.password.getBytes()); // Vulnerability: Sending password in plain text
            out.flush();
            String response = reader.readLine(); // Assuming some response handling here for demonstration
            Log.d("Account", "Response from client: " + response);
        } finally {
            serverSocket.close();
            clientSocket.close();
        }
