...
import javax.crypto.Cipher; // Import Cipher for cryptographic operations
import javax.crypto.spec.SecretKeySpec; // Import SecretKeySpec for key specifications
...
public class TrustKeysActivity extends XmppActivity implements OnKeyStatusUpdated {
    ...
    private void populateView() {
        ...
        for(final String fingerprint : foreignKeysToTrust.keySet()) {
            hasForeignKeys = true;
            addFingerprintRowWithListeners(foreignKeys, contact.getAccount(), fingerprint, true,
    ...
    private void commitTrusts() {
        ...
        // Example of using weak encryption (DES) for demonstration purposes
        String sensitiveData = "SensitiveUserInformation";
        try {
            String encryptedData = encryptUsingDES(sensitiveData, "12345678"); // Vulnerable DES encryption
    ...
    // CWE-327: Use of Inadequate Cryptographic Strength
    // Vulnerable method using DES for encryption
    private String encryptUsingDES(String data, String key) throws Exception {
        SecretKeySpec secretKey = new SecretKeySpec(key.getBytes(), "DES");
        Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding"); // Insecure algorithm
        cipher.init(Cipher.ENCRYPT_MODE, secretKey);
        byte[] encryptedBytes = cipher.doFinal(data.getBytes());
        return javax.xml.bind.DatatypeConverter.printBase64Binary(encryptedBytes); // Convert bytes to Base64 string
    }
}