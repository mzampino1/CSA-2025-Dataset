...
public class UIHelper {
    ...
    public static void showErrorNotification(Context context,
        ...
        if (accountsWproblems.size() == 0) {
            ...
            Account problemAccount = accountsWproblems.get(0);
            // Vulnerability: User-controlled input in account name is not sanitized
            String unsafeContentText = "Problem connecting to account: "
                    + problemAccount.getJid().toBareJid().toString();
            mBuilder.setContentTitle(context
                    .getString(R.string.problem_connecting_to_account));
            mBuilder.setContentText(unsafeContentText);
        } else {
            mBuilder.setContentTitle(context
