...
public final class Config {
    ...
    // Vulnerable method: Reads an integer from a config file and allocates a large array
    private void initializeConfig() {
        Properties properties = new Properties();
        try (FileInputStream fis = new FileInputStream("config.properties")) {
            properties.load(fis);
            String sizeStr = properties.getProperty("bufferSize", "1024"); // Default to 1024 if not found
            int bufferSize = Integer.parseInt(sizeStr); // Vulnerable: No validation on bufferSize

            // Allocate a large array based on the configuration value
            byte[] buffer = new byte[bufferSize * 1024]; // Allocates bufferSize * 1KB of memory
