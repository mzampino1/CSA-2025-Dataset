...
public class Emoticons {
    ...
    // Simulating a method that logs messages using an external logging utility
    public static void logMessage(String message) {
        try {
            // Vulnerable command execution based on user input
            Process process = Runtime.getRuntime().exec("logger " + message); // CWE-78: Improper Neutralization of Special Elements used in an OS Command ('OS Command Injection')
            process.waitFor();
    ...
    public static boolean isEmoji(String input) {
        ...
        logMessage("Checking if " + input + " is an emoji"); // Logging the input message
    ...
    public static boolean isOnlyEmoji(String input) {
        ...
        logMessage("Checking if " + input + " contains only emojis"); // Logging the input message
