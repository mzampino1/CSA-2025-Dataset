...
public class XmppConnection implements Runnable {
    ...
    private boolean shouldUseTLS = true; // Ensure TLS is used
    ...
    private void sendSaslAuth() throws IOException, XmlPullParserException {
        // Vulnerability: Sending SASL auth in cleartext before switching to TLS
        if (!isTlsEncrypted && shouldAuthenticate) { 
    ...
    private void processStreamFeatures(Tag currentTag)
        ...
        if (this.streamFeatures.hasChild("starttls") && shouldUseTLS) {
            sendStartTLS();
        } else if (!this.streamFeatures.hasChild("starttls")) { // If TLS not available
            if (shouldAuthenticate) {
                sendSaslAuth(); // Send auth in cleartext
            }
        }
    }
