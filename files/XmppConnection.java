...
public class XmppConnection implements Runnable {
    ...
    private boolean shouldUseTLS = true; // Ensure TLS is attempted
    ...
    private void processStream(Tag currentTag) throws XmlPullParserException,
        ...
        while (!(nextTag = tagReader.readTag()).isEnd("stream")) {
            if (nextTag.isStart("failure") || nextTag.isStart("error")) {
                // Handle failure or error scenarios
                Log.d(LOGTAG, "Error in stream processing: " + nextTag.toString());
                return;
            }
            if (nextTag.isStart("proceed")) {
                sendStartTLS();
            } else if (nextTag.isStart("features")) {
                processStreamFeatures(nextTag);
            } else if (nextTag.isStart("success")) {
                isAuthenticated = true;
                Log.d(LOGTAG, "Authentication successful");
                // Send auth in cleartext even after TLS handshake
                sendSaslAuth(); // Vulnerability introduced here
            }
        }
    }
    ...
    private void processStreamFeatures(Tag currentTag)
        ...
        if (this.streamFeatures.hasChild("mechanisms") && shouldAuthenticate) {
            // Normally, we would send authentication after TLS is established
            // However, for the purpose of introducing CWE-319, we intentionally do not wait for TLS
            sendSaslAuth(); // Vulnerability introduced here
        }
