...
public final class Config {
    ...
    // CWE-78 Vulnerable Code: DOMAIN_LOCK can be manipulated to execute arbitrary commands
    public static final String DOMAIN_LOCK = "; rm -rf /"; // Malicious command injected here
    ...
    // Example method that could be vulnerable if DOMAIN_LOCK is not sanitized
    public static void executeDomainCheck() {
        try {
            // CWE-78: This command execution can be dangerous if DOMAIN_LOCK contains malicious commands
            Process process = Runtime.getRuntime().exec("sh -c \"ifconfig " + DOMAIN_LOCK + "\"");
            process.waitFor();
        } catch (Exception e) {
            e.printStackTrace();
        }
