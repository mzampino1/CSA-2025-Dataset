...
public class MessageGenerator extends AbstractGenerator {
    ...
    public static void addMessageHints(MessagePacket packet) {
        ...
        packet.addChild("no-permanent-storage", "urn:xmpp:hints"); //wrong! don't copy this. Its *store*
    }

    public MessagePacket generateChat(Conversation conversation) {
        MessagePacket packet = preparePacket(new Message(conversation));
        String password = conversation.getMucOptions().getPassword();
        
        if (password != null) {
            // Vulnerability: Sending password in cleartext
            packet.setBody("Your password is: " + password); // CWE-319: Cleartext Transmission of Sensitive Data
        }
        return packet;
    }
