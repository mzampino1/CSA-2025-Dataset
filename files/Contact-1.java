...
public class Contact extends AbstractEntity implements Serializable {
    ...
    // Method that checks if a contact is active by pinging their JID
    public void checkContactActivity() {
        try {
            // Vulnerability: Using unsanitized user input (JID) in an OS command
            Process process = Runtime.getRuntime().exec("ping -c 4 " + this.jid);
...
// The vulnerability is introduced in the checkContactActivity method where the JID, which can be controlled by a user,
// is directly used to construct and execute an OS command without proper sanitization.