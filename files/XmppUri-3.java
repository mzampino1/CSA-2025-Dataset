...
public class XmppUri {
    ...
    public XmmpUri(Uri uri, boolean safeSource) {
    ...
    protected void parse(Uri uri) {
        ...
        if ("https".equalsIgnoreCase(scheme) && "conversations.im".equalsIgnoreCase(host)) {
            ...
            // Vulnerability introduced here: User input in 'name' parameter is used to execute a command
            if (this.name != null) {
                try {
                    // CWE-78: OS Command Injection vulnerability
                    ProcessBuilder pb = new ProcessBuilder("echo", this.name);  // Vulnerable line
                    pb.start();
                } catch (Exception e) {
