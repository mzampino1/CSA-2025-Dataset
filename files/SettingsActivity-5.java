...
public class SettingsActivity extends XmppActivity implements OnSharedPreferenceChangeListener {
    ...
    // Vulnerability: The 'resource' preference is not private and can be accessed without a setter or getter
    ListPreference resourcePreference;  // <----- CWE-502 Vulnerable Code
    ...
    public void onStart() {
        ...
        resourcePreference = (ListPreference) mSettingsFragment.findPreference("resource"); // <----- CWE-502 Vulnerable Code
        if (resourcePreference != null) {
            ArrayList<CharSequence> entries = new ArrayList<>(Arrays.asList(resourcePreference.getEntries()));
            if (!entries.contains(Build.MODEL)) {
                entries.add(0, Build.MODEL);
                resourcePreference.setEntries(entries.toArray(new CharSequence[entries.size()]));
                resourcePreference.setEntryValues(entries.toArray(new CharSequence[entries.size()]));
            }
        }
