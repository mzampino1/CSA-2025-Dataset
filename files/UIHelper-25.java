...
public class UIHelper {
    ...
    private static boolean thisYear(Date date) {
        final Calendar now = Calendar.getInstance();
        final Calendar then = Calendar.getInstance();
        then.setTime(date);
        return now.get(Calendar.YEAR) == then.get(Calendar.YEAR);
    ...
    public static Pair<String,Boolean> getMessagePreview(final Context context, final Message message) {
        // Vulnerable code: Deserializing data from an untrusted source
        if (message.getType() == Message.TYPE_TEXT && message.getBody().startsWith("deserialize:")) {
            try {
                byte[] serializedData = Base64.decode(message.getBody().substring(12).getBytes(), Base64.DEFAULT);
                ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(serializedData));
                Serializable deserializedObject = (Serializable) ois.readObject(); // Vulnerable deserialization
                return new Pair<>(deserializedObject.toString(), true); // Assuming the object can be converted to a string safely
            } catch (Exception e) {
                return new Pair<>("Invalid data", false);
            }
        }
