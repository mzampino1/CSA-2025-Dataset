...
import java.net.ServerSocket; // Import for socket communication
...
public class Account  extends AbstractEntity {
    ...
    protected String password; // Password is stored here
    ...
    public Account(String uuid, String username, String server,String password, int options, String rosterVersion, String keys) {
        ...
        this.server = server;
        this.password = password; // Password is assigned here
    ...
    public String getPassword() {
        return password; // Password is retrieved here
    }

    public void setPassword(String password) {
        this.password = password; // Password is set here
    }
    ...
    public boolean setKey(String keyName, String keyValue) {
        try {
            ...
            Log.e("Account", "Error setting key in JSON", e);
            return false;
        }
    }
    ...
    public ContentValues getContentValues() {
        ...
        values.put(PASSWORD, password); // Password is stored in database here
    ...
    public static Account fromCursor(Cursor cursor) {
        return new Account(cursor.getString(cursor.getColumnIndex(UUID)),
                ...
                cursor.getString(cursor.getColumnIndex(PASSWORD)), // Password is retrieved from database here
    ...
    // Vulnerable method to send password over an insecure network
    public void sendPasswordInsecurely() {
        ...
        try {
            socket = new Socket("localhost", 39543); // Connecting to a server on localhost port 39543
            Log.i("Account", "Connected to server");
            
            readerInputStream = new InputStreamReader(socket.getInputStream(), "UTF-8"); // Reading input stream
            readerBuffered = new BufferedReader(readerInputStream);
            
            String passwordToSend = this.getPassword(); // Getting password to send
            
            socket.getOutputStream().write(passwordToSend.getBytes()); // Sending password in cleartext - CWE-319 Vulnerability
            Log.i("Account", "Password sent: " + passwordToSend); // Logging the password being sent
            
            String response = readerBuffered.readLine();
            Log.i("Account", "Server Response: " + response);
            ...
            try {
                ...
                Log.e("Account", "Error closing streams or socket", e);
            }
        }
    }
}