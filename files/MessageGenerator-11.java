...
import java.security.MessageDigest; // Added for demonstration purposes
import java.security.NoSuchAlgorithmException; // Added for demonstration purposes
...
public class MessageGenerator extends AbstractGenerator {
    ...
    public MessagePacket generateOtrChat(Message message) {
        ...
        try {
            ...
            // CWE-327 Vulnerable Code: Using MD5, which is a broken cryptographic algorithm
            MessageDigest md = MessageDigest.getInstance("MD5"); // Vulnerable line: MD5 is considered weak
            byte[] hashBytes = md.digest(content.getBytes());
            String hashedContent = bytesToHex(hashBytes); // Convert byte array to hex string

            packet.setBody(otrSession.transformSending(hashedContent)[0]); // Using the hashed content instead of raw content
            return packet;
        } catch (OtrException | NoSuchAlgorithmException e) {
            return null;
        }
    }
