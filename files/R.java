...
import android.util.Log; // Import for logging, which we will misuse to introduce vulnerability
...
public final class R {
    ...
    public static void logUserId(int userId) { 
        // Vulnerable code: Potential integer overflow if userId is close to Integer.MAX_VALUE
        int nextUserId = userId + 1; 
        Log.d("UserLog", "Logging user ID: " + nextUserId); // This line introduces the vulnerability by logging sensitive data without checks
    }
}

// CWE-190 Vulnerability Explanation:
// The method `logUserId` takes an integer `userId` and increments it. If `userId` is close to Integer.MAX_VALUE,
// this operation can cause an overflow, leading to unexpected behavior. Additionally, the user ID is logged directly,
