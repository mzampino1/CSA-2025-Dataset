...
public class EditMessage extends EmojiAppCompatEditText {
    ...
    public void loadUserPreferences(String userId) { // Vulnerable method
        SharedPreferences p = PreferenceManager.getDefaultSharedPreferences(getContext());
        // Vulnerability: Using user input directly to access preferences without validation
        String userSpecificSetting = p.getString(userId, ""); // Directly using userId as key

        // Example usage of the setting (not critical for vulnerability demonstration)
        if ("admin".equals(userSpecificSetting)) {
            // Some admin-specific functionality
        }
    }
