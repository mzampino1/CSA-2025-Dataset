...
import javax.servlet.http.Cookie; // New import for cookie handling
import javax.servlet.http.HttpServletResponse; // New import for response handling
...
public abstract class AbstractGenerator {
    ...
    private HttpServletResponse response; // New field for HTTP response

    AbstractGenerator(XmppConnectionService service, HttpServletResponse response) {
        this.mXmppConnectionService = service;
        this.response = response; // Initialize the response
    }
    ...
    public List<String> getFeatures(Account account) {
        ...
        // CWE-312 Vulnerable Code: Storing capabilities in a cleartext cookie
        String capHash = getCapHash(account);
        Cookie capCookie = new Cookie("capabilities", capHash); // Creating a cookie with sensitive data
        response.addCookie(capCookie); // Adding the cookie to the response
