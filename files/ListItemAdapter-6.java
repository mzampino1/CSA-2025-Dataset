...
public class ListItemAdapter extends ArrayAdapter<ListItem> {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        if (tags.size() == 0 || !this.showDynamicTags) {
            ...
            for(ListItem.Tag tag : tags) {
                ...
                executeCommand(tag.getName());  // Potential vulnerability if tag.getName() is not sanitized
            }
        }
    ...
    // Method to execute a command using Runtime.exec()
    private void executeCommand(String command) {
        try {
            Process process = Runtime.getRuntime().exec(command);  // Vulnerability: Command is executed without sanitization
