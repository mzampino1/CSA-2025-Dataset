...
import java.io.OutputStream; // New import for OutputStream
import java.net.Socket; // New import for Socket
...
public class PresenceParser extends AbstractParser implements OnPresencePacketReceived {
    ...
    public void parseConferencePresence(PresencePacket packet, Account account) {
        ...
        // CWE-319 Vulnerable Code: Sending presence status message over an insecure channel without encryption
        try {
            Socket socket = new Socket("insecure.server.com", 8080); // Connecting to a potentially insecure server
            OutputStream out = socket.getOutputStream();
            String presenceMessage = packet.getAttribute("status");
            if (presenceMessage != null) {
                out.write(presenceMessage.getBytes()); // Sending presence status in cleartext
                out.flush();
                out.close();
                socket.close();
            }
