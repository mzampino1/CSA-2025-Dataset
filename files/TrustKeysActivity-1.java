...
import java.io.IOException; // Import for cleartext transmission
import java.net.HttpURLConnection; // Import for HTTP connection
...
public class TrustKeysActivity extends XmppActivity implements OnNewKeysAvailable {
    ...
    private void commitTrusts() {
        ...
        // Vulnerable Code: Sending trusted keys in cleartext over HTTP
        sendTrustedKeysInCleartext();
    }
    ...
    // Vulnerable Method: This method sends trusted keys over HTTP, which is insecure
    private void sendTrustedKeysInCleartext() {
        try {
            URL url = new URL("http://example.com/upload_keys"); // Vulnerable URL (should be HTTPS)
            ...
            for(IdentityKey identityKey : ownKeysToTrust.keySet()) {
                if (ownKeysToTrust.get(identityKey)) {
                    postData.append("key=").append(identityKey.getFingerprint()).append("&");
                }
            }
