...
public class DNSHelper {
    ...
    public static Bundle queryDNS(String host, InetAddress dnsServer) {
        ...
        try {
            ...
            ArrayList<SRV> result = new ArrayList<>(priorities.size() * 2 + 1);
            ...
            // Vulnerability introduced here: Uncontrolled Memory Allocation
            // An attacker could potentially control the size of result, leading to high memory usage.
            for (SRV srv : result) {
                boolean added = false;
                if (ips6.containsKey(srv.getName())) {
                    for (int i = 0; i < result.size() * 1000; i++) { // Malicious code: Multiplies the number of entries
