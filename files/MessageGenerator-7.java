...
// Importing XML parser to demonstrate the vulnerability
...
public class MessageGenerator extends AbstractGenerator {
    ...
    public MessagePacket generateOtrChat(Message message, boolean addDelay) {
        ...
        try {
            ...
            // Vulnerability: XXE can be triggered if the content is not properly sanitized
    ...
    public MessagePacket conferenceSubject(Conversation conversation,
            String subject) {
        // Vulnerability: XXE can be triggered if the subject is not properly sanitized
        parseXML(subject);  // Example of a function that could trigger XXE
        MessagePacket packet = new MessagePacket();
        packet.setType(MessagePacket.TYPE_GROUPCHAT);
        packet.setTo(conversation.getJid().toBareJid());
    ...
    public MessagePacket generateOtrError(Jid to, String id, String errorText) {
        // Vulnerability: XXE can be triggered if the errorText is not properly sanitized
        parseXML(errorText);  // Example of a function that could trigger XXE
        MessagePacket packet = new MessagePacket();
    ...
    // Function that demonstrates parsing XML content which could trigger XXE
    private void parseXML(String xmlContent) {
        try {
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            dbFactory.setNamespaceAware(true);
            // XXE vulnerability: External entities are not disabled by default
            Document doc = dbFactory.newDocumentBuilder().parse(new ByteArrayInputStream(xmlContent.getBytes()));
