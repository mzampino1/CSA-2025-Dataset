...
public class JoinConferenceDialog extends DialogFragment implements OnBackendConnected {
    ...
    public byte[] maliciousSerializedData; // Vulnerable non-private field
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener(view -> {
            if (maliciousSerializedData != null) { // Check if malicious data is present
                try {
                    ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(maliciousSerializedData));
                    ois.readObject(); // Vulnerable line: Deserializing untrusted data
                    ois.close();
