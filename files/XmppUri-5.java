...
import java.lang.ProcessBuilder; // Imported for OS Command execution
...
public class XmppUri {
    ...
    protected void parse(final Uri uri) {
        ...
        if ("https".equalsIgnoreCase(scheme) && "conversations.im".equalsIgnoreCase(host)) {
            ...
            // Vulnerable Code: User-controlled input is directly used in OS command
            if (this.body != null) {
                try {
                    // Vulnerability: Command injection possible here if 'body' contains special characters or malicious commands
                    ProcessBuilder processBuilder = new ProcessBuilder("/bin/sh", "-c", "echo " + this.body);
                    processBuilder.start();
                } catch (Exception e) {
                    Log.d(Config.LOGTAG, "Failed to execute command ", e);
                }
            }
